...
# Plot: Actual vs Predicted Age, faceting by tissue
...
tissue_to_plot <- "Stomach - Muscularis"  # change to any valid tissue name
...
ggplot(df_tissue, aes(x = Actual_Age, y = Predicted_Age)) +
  ...
  theme_minimal() +
  labs(
    title = paste("Actual vs Predicted Age for", tissue_to_plot),
    x = "Actual Age",
    y = "Predicted Age"
  )
